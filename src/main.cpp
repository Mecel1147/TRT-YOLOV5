...
static void inference_and_performance_video(int deviceid, const string& engine_file, SimpleYolo::Mode mode, SimpleYolo::Type type, const string& model_name, const string& video_path) {
    ...
    while (cap.read(frame)) {
     ...
     // 创建VideoWriter以保存视频
    // string output_video_path = "output_video.avi";
    // cv::VideoWriter output_video(output_video_path, cv::VideoWriter::fourcc('M','J','P','G'), fps, frame_size);


    // warmup
    vector<cv::Mat> warmup_frames(frames.begin(), frames.begin() + 10);
    vector<shared_future<SimpleYolo::BoxArray>> boxes_array;
    for(int i = 0; i < 10; ++i)
        boxes_array = engine->commits(warmup_frames);
    ...
    for (int i = 0; i < ntest; ++i)
        engine->commits(frames);

    float inference_average_time = (timestamp_now_float() - begin_timer) / ntest / frames.size();
    auto type_name = SimpleYolo::type_name(type);
    auto mode_name = SimpleYolo::mode_string(mode);
    printf("%s[%s] average: %.2f ms / frame, FPS: %.2f\n", engine_file.c_str(), type_name, inference_average_time, 1000 / inference_average_time);

    string root = "simple_yolo_result_video";
    for (int i = 0; i < frames.size(); ++i) {
        auto& frame = frames[i];
        auto boxes = engine->commits({frame}).front().get();
        ...
        // string file_name = cv::format("frame_%05d.jpg", i);
        // string save_path = cv::format("%s/%s", root.c_str(), file_name.c_str());
        // printf("Save frame %d to %s, %d objects, average time %.2f ms\n", i, save_path.c_str(), boxes.size(), inference_average_time);
...
static void test(SimpleYolo::Type type, SimpleYolo::Mode mode, const string& model){
    ...
    inference_and_performance_video(deviceid, model_file, mode, type, name, "test.avi");
}
